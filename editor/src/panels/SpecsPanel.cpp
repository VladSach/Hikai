#include "SpecsPanel.h"

void SpecsPanel::init()
{
    is_open_ = false;
}

void SpecsPanel::deinit()
{
}

void SpecsPanel::display()
{
    if (!is_open_) { return; }

    hk::imgui::push([&](){
        if (ImGui::Begin("Specs", &is_open_)) {
...
void SpecsPanel::addMonitorSpec()
{
    if (ImGui::CollapsingHeader("Monitors")) {
        const auto &infos = hk::spec::system().monitors;
        ...
        for (auto &info : infos) {
            ...
            ImGui::Text("Scale: %i%%", static_cast<u32>(info.scale * 100.f + .5f));
...
void SpecsPanel::addCPUSpec()
{
    if (ImGui::CollapsingHeader("CPU")) {
        const auto &info = hk::spec::cpu();
...
void SpecsPanel::addGPUSpec()
{
    if (ImGui::CollapsingHeader("Physical Device")) {
        for (u32 i = 0; i < hk::vkc::adapter_count(); ++i) {
            const auto &device = hk::vkc::adapter_info(i);
            const auto &info = hk::spec::adapter(i);
            if (ImGui::TreeNode(info.name.c_str())) {
                ImGui::Text("Type: %s", hk::spec::to_string(info.type));
                ImGui::Text("Vendor: %s", hk::spec::to_string(info.vendor));

                ImGui::Text("API: %s", hk::spec::to_string(info.api.type));
                ImGui::Text("API Version: %s", info.api.version.c_str());
                ImGui::Text("Driver Version: %s", info.api.driver_version.c_str());
                ...
                if (ImGui::TreeNode("Queue Families")) {
                    ...
                    if (ImGui::BeginTable("Families", 6, flags)) {
                        ...
                        ImGui::TableHeadersRow();
                        for (u32 j = 0; j < device.families.size(); ++j) {
                            auto &family = device.families.at(j);
                            ImGui::TableNextRow();
                            ImGui::TableSetColumnIndex(0);
                            ImGui::Text("%u", j);
                            ImGui::TableSetColumnIndex(1);
    ...
    if (ImGui::CollapsingHeader("Logical Device")) {
        ...
        if (ImGui::TreeNode("Chosen Queues")) {
            ...
            if (ImGui::BeginTable("Graphics Family", 6, flags)) {
                ...
                hk::QueueFamily &family = info.graphics_family;
                ImGui::TableNextRow();
                ImGui::TableSetColumnIndex(0);
            ...
            if (ImGui::BeginTable("Compute Family", 6, flags)) {
                ...
                hk::QueueFamily &family = info.compute_family;
                ImGui::TableNextRow();
                ImGui::TableSetColumnIndex(0);
                ImGui::Text("%u", family.index_);
                ImGui::TableSetColumnIndex(1);
            ...
            if (ImGui::BeginTable("Transfer Family", 6, flags)) {
                ...
                hk::QueueFamily &family = info.transfer_family;
                ImGui::TableNextRow();
                ImGui::TableSetColumnIndex(0);
                ImGui::Text("%u", family.index_);
                ImGui::TableSetColumnIndex(1);
