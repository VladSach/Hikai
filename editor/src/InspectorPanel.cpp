...
void InspectorPanel::addMaterialInfo(hk::SceneNode *node)
{
    ...
    if (ImGui::CollapsingHeader("Material", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::TreeNodeEx("Constants", ImGuiTreeNodeFlags_DefaultOpen)) {
            ImGui::ColorEdit4("Color", &material.data.cons.color.x);

            ImGui::InputFloat("Alpha",        &material.data.cons.alpha,        .0f, 1.f, "%.3f");
            ImGui::InputFloat("Shininess",    &material.data.cons.shininess,    .0f, 1.f, "%.3f");
            ImGui::InputFloat("Reflectivity", &material.data.cons.reflectivity, .0f, 1.f, "%.3f");
            ImGui::TreePop();
        }
