...
HKAPI b8 read_file(const hk::string &path, hk::vector<u8> &out);

// HKAPI b8 write_file(const hk::string &path, const hk::string text);
// HKAPI b8 create_file(const std::string &path);

HKAPI b8 find_file(const hk::string &root, const hk::string &target,
                   hk::string *out = nullptr);
