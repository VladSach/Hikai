...
REM For Visual Studio 2017 and later
REM calls vswhere

for /f "usebackq tokens=*" %%i in (` ^
        "%ProgramFiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe" ^
        -latest -products * -requires Microsoft.Component.MSBuild -property ^
        installationPath`) do (
...
rem For versions prior to Visual Studio 2017
rem SOURCE: https://gist.github.com/juntalis/10433731
rem
...
set VCYEAR=
set VCROOT=
for /L %%V in (%_VC_MIN_VERSION_%, ^
               %_VC_STEP_VERSION_%, ^
               %_VC_MAX_VERSION_%) do @call "%~f0" %%~V
